...
$sql1 = "select * from sale_invoices INNER JOIN customers ON sale_invoices.customer_id = customers.customer_id where sale_user_name = '$user_name'or customers.user_name='$user_name'ORDER BY sale_date DESC";
...
if (mysqli_num_rows($result1)>0){
   // $status = "ok";
    ...
    {
        array_push($response,array("response"=>'ok',"sale_user_name"=>$row['sale_user_name'],"id"=>$row['id'],"user_name"=>$row['sale_user_name'],"voucher_number"=>$row['voucher_number'],"sale_date"=>$row['sale_date'],"new_total_kyat"=>$row['total_kyat'],"new_total_pal"=>$row['total_pal'],"new_total_yae"=>$row['total_yae'],"total_qualtity"=>$row['qty'],
